...
def get_learners_data(last_updated_at=None):
    ...
    qs.purpose,
    qs.x_id AS qset_uid,
    qs.taxonomy->'l1_skill'->'name'->>'en' AS l1_skill,
...
def get_grades():
    query = "SELECT id, cm.name->>'en' AS grade FROM class_master cm"
    return execute_query_with_retry(query)
...
def get_repository_names():
    query = "SELECT DISTINCT(name->>'en') AS repo_name FROM repository"
    return execute_query_with_retry(query)


def get_l2_skills():
    query = "SELECT DISTINCT(taxonomy->'l2_skill'->0->'name'->>'en') AS l2_skill FROM question_set"
    return execute_query_with_retry(query)


def get_l3_skills():
    query = "SELECT DISTINCT(taxonomy->'l3_skill'->0->'name'->>'en') AS l3_skill FROM question_set"
    return execute_query_with_retry(query)
...
def get_data(key):
    if key == ALL_LEARNER_DATA_KEY:
        ...
        all_schools_list = get_cached_data(key, fetch_all_data)
        return all_schools_list["school_name"].sort_values().unique()
    elif key == ALL_QSET_TYPES_KEY:
        all_qset_types_list = get_cached_data(key, fetch_all_data)
        return all_qset_types_list["purpose"].sort_values().unique()
    elif key == ALL_REPOSITORY_NAMES_KEY:
        all_repository_names_list = get_cached_data(key, fetch_all_data)
        return all_repository_names_list["repo_name"].sort_values().unique()
    elif key == ALL_L2_SKILLS_KEY:
        all_l2_skills_list = get_cached_data(key, fetch_all_data)
        return all_l2_skills_list["l2_skill"].sort_values().unique()
    elif key == ALL_L3_SKILLS_KEY:
        all_l3_skills_list = get_cached_data(key, fetch_all_data)
        return all_l3_skills_list["l3_skill"].sort_values().unique()
